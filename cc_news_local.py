...
_DOWNLOAD_URL = (
    "https://storage.googleapis.com/huggingface-nlp/datasets/cc_news/cc_news.tar.gz"
)
...
class CCNewsConfig(datasets.BuilderConfig):
    ...
    def __init__(self, **kwargs):
        ...
        **kwargs: keyword arguments forwarded to super.
        """
        super(CCNewsConfig, self).__init__(
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN, gen_kwargs={"directory": arch_path}
            ),
        ]
        ...
        for article_file_path in article_files:
            with open(article_file_path, mode="r", encoding="utf-8") as f:
                ...
                yield id_, {
                    ...
                    "text": article["maintext"].strip()
                    if article["maintext"] is not None
                    else "",
                    "domain": article["source_domain"].strip()
                    if article["source_domain"] is not None
                    else "",
                    "date": article["date_publish"].strip()
